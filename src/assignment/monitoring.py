        ...
        if self.rate_limit_hits > self.rate_limit_hit_threshold:
            self.alerts.append(
                Alert(
                    ...
                    message=f"Rate limit threshold exceeded: {self.rate_limit_hits} hits (threshold: {self.rate_limit_hit_threshold})",
                )
            )
        ...
        if judge_fail_rate > self.judge_fail_rate_threshold:
            self.alerts.append(
                Alert(
                    ...
                    message=f"High judge failure rate detected: {judge_fail_rate:.2%} (threshold: {self.judge_fail_rate_threshold:.2%})",
                )
            )
